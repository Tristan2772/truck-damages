...
const canManageReport = computed(() => {
  ...
  return ((isManager.value && report.value.repairs.length > 0) || ((Number(authStore.user.id) === report.value.userId) && report.value.repairs.length === 0));
});
    ...
    <div v-if="route.name === 'damages-trucks-vin-reports-id' && report && !loading">
      <div class="flex flex-col">
        <div class="w-full flex flex-col gap-2 items-start z-10 pl-4">
          <div class="flex flex-col flex-1 gap-2 justify-start items-start pt-5">
            <div class="flex gap-4 text-sm italic text-gray-500">
              ...
              <p v-if="isTruckArchived" class="badge badge-warning">
                Archived Truck
              </p>
            </div>
        ...
        <div class="p-4 flex w-full justify-center gap-2 flex-wrap">
          ...
          <div v-if="report.repairs.length > 0" class="w-full card-body text-center flex flex-col items-center justify-center gap-4">
            <NuxtLink
              :to="{ name: 'damages-trucks-vin-reports-id-repairs', params: { vin: route.params.vin, id: report.id } }"
              class="btn btn-secondary w-60"
            >
              View Repairs ({{ report.repairs.length }})
              <AppWrenchIcon />
            </NuxtLink>
          </div>
